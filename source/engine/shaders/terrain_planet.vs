...
void main()
{
    ...
    // At low n the sigmoid dominates, flattening valley floors.
    // At high n the raw value dominates, preserving noisy peaks.
    float sig = 1.0 / (1.0 + exp(-n * 5.0));
    float t = clamp(n, 0.0, 1.0);   // blend weight: 0 at low n, 1 at high n
    n = mix(sig, n, t);
