...
void main()
{
    ...
    // More bands for subtle lunar shading
    if (h < 0.15) {
        object_color = mix(vec3(0.05), vec3(0.18), h / 0.15); // very dark -> dark
    } else if (h < 0.35) {
        object_color = mix(vec3(0.18), vec3(0.33), (h - 0.15) / 0.20);
    } else if (h < 0.55) {
        object_color = mix(vec3(0.33) * 0.5, vec3(0.52), (h - 0.35) / 0.20);
    } else if (h < 0.75) {
        object_color = mix(vec3(0.52) * 0.5, vec3(0.72), (h - 0.55) / 0.20);
    } else if (h < 0.90) {
        object_color = mix(vec3(0.72) * 0.5, vec3(0.88), (h - 0.75) / 0.15);
    } else {
        object_color = mix(vec3(0.88) * 0.5, vec3(1.0), (h - 0.90) / 0.10); // light -> near-white
    }
}
    ...
    if (planet_type == 1){
        if (height_normalized < 0.5) {
            ...
            object_color = mix(vec3(0.8, 0.4, 0.2), vec3(1.0, 0.8, 0.6)*0.5, (height_normalized - 0.5) / 0.5);
        }
    }
