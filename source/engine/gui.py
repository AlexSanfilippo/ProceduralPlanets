...
class GUI:
    ...
    def toggle_context_status(self, context_id_to_update):
        try:
            self.switch_context_status(
                context_id=context_id_to_update,
                status=not(self.context_id_to_status[context_id_to_update]),
            )
        except KeyError:
            print(f'WARNING: \'{context_id_to_update}\' is not a GUI context.')
