...
planet = PlanetMesh(
    ...
    position=vec3(-60.0, 0.0, -30.0),
    scale=40.0,
    projection=projection,
    noise_scale=5.25,
    displacement_amplitude=4.4,
    subdivisions=4,
    amplitude=1.0
)
...
def next_seed():
    ...
    planet_settings["seed"] = random() * 1000
...
def update_planet_lights(shader):
    ...
    glUniform3fv(glGetUniformLocation(shader, "point_light.position"), 1, light_cubes[0].get_pos())

    glUniform3fv(glGetUniformLocation(shader, "point_light.ambient"), 1, [1.0, 1.0, 1.0])
    glUniform3fv(glGetUniformLocation(shader, "point_light.diffuse"), 1, [1.0, 1.0, 1.0])
    glUniform3fv(glGetUniformLocation(shader, "point_light.specular"), 1, [1.0, 1.0, 1.0])
    glUniform1fv(glGetUniformLocation(shader, "point_light.constant"), 1, 1)
    glUniform1fv(glGetUniformLocation(shader, "point_light.linear"), 1, 0.014)
    glUniform1fv(glGetUniformLocation(shader, "point_light.quadratic"), 1, 0.00007)
...
triangle_pos_norm = TriangleIndexed(shader_program=shader_program_pos_normal, position=vec3(-15.0, 0.0, 0.0), scale=10.0, projection=projection)
cube_pos_norm = Cube(shader_program=shader_program_pos_normal, position=vec3(0.0, 0.0, 0.0), scale=10.0, projection=projection)
icosphere_pos_norm = Icosphere(shader_program=shader_program_pos_normal, position=vec3(15.0, 0.0, 0.0), scale=10.0, projection=projection, subdivisions=3)
triangle_subdivided = TriangleSubdivided(shader_program=shader_program_pos_normal, position=vec3(30.0, 0.0, 0.0), scale=10.0, projection=projection)
icosphere_subdivided = IcosphereSubdivided(
    # shader_program=shader_program_pos_normal,
    shader_program=shader_point_light,
    position=vec3(60.0, 0.0, -30.0),
    scale=40.0,
    projection=projection,
    face_subdivisions=[1 if i % 2 == 0 else 3 for i in range(20)],
)


shaders_lighting = [icosphere_subdivided.shader_program]
...
while not glfw.window_should_close(window):
    ...
    triangle_pos_norm.draw(view_matrix=view)
    cube_pos_norm.draw(view_matrix=view)
    icosphere_pos_norm.draw(view_matrix=view)
    triangle_subdivided.draw(view_matrix=view)
    icosphere_subdivided.draw(view_matrix=view)
    planet.draw(view_matrix=view)


    if test_subdivide:
        # triangle_subdivided.subdivide()
        # icosphere_subdivided.update_subdivisions_by_player(active_camera.camera_pos, regenerate=True, max_level=5)
        icosphere_subdivided.adaptive_subdivide(
            subdivide_condition=make_player_level_condition(
                player_position=active_camera.camera_pos,
                center=icosphere_subdivided.position,
                max_sublevel=get_subdivision_level(
                    player_pos=active_camera.camera_pos,
                    planet_pos=icosphere_subdivided.position,
                    planet_radius=icosphere_subdivided.scale,
                    max_subdivision=12
                ),
                angle_sensitivity=1,
            ),
